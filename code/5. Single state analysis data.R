...
##### 1. Load all data for single state #####

# year/zip confounders
yr_zip_confounders <- read_fst(paste0(dir_data, "denom/year_zip_confounders.fst"), 
                               as.data.table = TRUE)
yr_zip_confounders <- yr_zip_confounders[statecode == state]
setkey(yr_zip_confounders, year, zip)
state_zips <- sort(unique(yr_zip_confounders$zip))
...
for (yr in 2009:2016) {
  ...
  dt <- read_fst(paste0(dir_data, "denom/qid_data_", yr, ".fst"), 
                 as.data.table = TRUE)
  dt <- dt[zip %in% state_zips & # restrict to state zipcodes
...
setkey(qid_dat, year, zip, qid)
write_fst(qid_dat, paste0(dir_data, "analysis/state", state, "_", AD_ADRD, "_", 
                          code_type, "_qid.fst"))
...
for (e in exposures) {
  ...
  write_fst(exp_dat, paste0(dir_data, "analysis/state", state, "_", AD_ADRD, "_", 
                            code_type, "_", e, ".fst"))
  rm(exp_dat)
  cat(" complete.")
}
