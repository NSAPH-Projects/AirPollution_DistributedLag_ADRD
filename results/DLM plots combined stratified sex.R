...
for (exp in c("pm25", "no2", "ozone")) {
  for (sex in 0:1) {
    ...
    plot_dat[[paste0(exp,sex)]] <- data.table(lag = seq(1, 10, by = 0.2),
                                  est = exp(fit),
                                  low = exp(fit - se),
                                  high = exp(fit + se),
                                  cum = exp(fit_cum),
...
ggplot(plot_dat, aes(x = lag, y = est, ymin = low, ymax = high, color = sex, fill = sex)) +
  ...
  labs(x = "Years prior", y = "Odds ratio (NAAQS vs AQG)", fill = "", color = "")

ggplot(plot_dat[lag == 1], 
       aes(x = exp, y = cum, ymin = cum_low, ymax = cum_high, color = sex)) +
  geom_hline(yintercept = 1, color = "black", size = 1) +
